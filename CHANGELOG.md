...
### 改进
- **WebUI 配置补全**: 配置页覆盖 `_conf_schema.json` 全部配置项，补齐认证 / 命令执行 / 抖动被动并入 / 画图等 12 项缺失配置
  - 配置项按功能分组（上下文、工具、记忆、主动、抖动、分段、清洗、画图、WebUI），每组带彩色顶条与说明
  - 布尔配置统一改为滑动开关，体验更直观
  - 敏感凭证（`web_password`、`image_gen_api_key`）GET 接口不再回显真实值，仅返回 `*_set` 布尔标记；前端显示「已设置 / 未设置」，留空保存即不修改
  - 修改需重启生效的配置（端口、数据库、画图、记忆 / 主动总开关等）时，保存前弹出确认提示
- **WebUI 刷新增强**: 人格、配置页补齐刷新按钮；记忆 / 主动回复页刷新改为同时刷新「会话下拉框 + 已选会话数据」，修复原先只刷下拉框不刷内容的问题
  - 刷新按钮统一加入「刷新中…」禁用态反馈
- **F5 状态保持**: 刷新页面（F5）后自动恢复到上次的标签页，并恢复记忆 / 主动回复页已选会话与会话列表页码（基于 `localStorage`）
- **WebUI 美化**: 侧边栏激活项加彩色竖条、配置分组卡片化、记忆面板计数徽章、空状态圆点装饰、按钮按压反馈等（纯文字风格，未引入 Emoji / 图标）

### 修复
- **token-stats-bar 背景变量未定义**: 新增 `--surface` 别名指向 `--bg-card-hover`，修复 Token 统计条背景透明问题
